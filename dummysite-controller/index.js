...
function streamWatch(path, onEvent, onError) {
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      const msg = status >= 200 && status < 300 ? 'watch stream ended' : `watch stream ended with status ${status}`;
...
function cmName(name) { return `dummysite-${name}`; }
function appLabel(name) { return `dummysite-${name}`; }

// No ConfigMap needed with initContainer + emptyDir approach

function proxyEnv() {
  const env = [];
  if (process.env.HTTP_PROXY) env.push({ name: 'HTTP_PROXY', value: process.env.HTTP_PROXY });
  if (process.env.HTTPS_PROXY) env.push({ name: 'HTTPS_PROXY', value: process.env.HTTPS_PROXY });
  if (process.env.NO_PROXY) env.push({ name: 'NO_PROXY', value: process.env.NO_PROXY });
  return env;
}
...
async function ensureDeployment(ns, name, url) {
      ...
      template: {
        ...
        spec: {
          initContainers: [{
            ...
            command: ['sh', '-c'],
            args: ['set -e; echo "Fetching $URL"; curl -fsSL "$URL" -o /workdir/index.html'],
  ...
  try {
    const existing = await k8sRequest('get', `/apis/apps/v1/namespaces/${ns}/deployments/${depName}`);
    payload.metadata.resourceVersion = existing.metadata && existing.metadata.resourceVersion;
    await k8sRequest('put', `/apis/apps/v1/namespaces/${ns}/deployments/${depName}`, payload);
  } catch (e) {
    if (String(e.message).includes('404')) {
      await k8sRequest('post', `/apis/apps/v1/namespaces/${ns}/deployments`, payload);
    } else {
      throw e;
    }
  }
...
async function deleteResources(ns, name) {
  ...
  const del = async (path) => {
    try { await k8sRequest('delete', path); } catch (e) { /* ignore */ }
  };
...
function startWatch() {
  const qs = `watch=true&timeoutSeconds=300&allowWatchBookmarks=true${lastRV ? `&resourceVersion=${encodeURIComponent(lastRV)}` : ''}`;
  const path = `/apis/stable.dwk/v1/dummysites?${qs}`;
  streamWatch(path, (evt) => {
    const type = evt.type;
